...
def register_user(client, email, name="tester", amount=0):
    ...
    if amount:
        client.post(f"/api/users/{auth['user']['id']}/wallet/recharge", json={"amount": amount})
...
def test_full_rent_and_return_flow(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        scan = client.post("/api/hardware/qr/scan", json={"token": qr["token"]}).json()
        assert scan["unlock"] is True
        assert scan["action"] == "rent"

        pickup = client.post("/api/hardware/slots/1/sensor", json={"present": False}).json()
        ...
        client.post("/api/hardware/qr/scan", json={"token": return_qr["token"]})
        returned = client.post("/api/hardware/slots/1/sensor", json={"present": True}).json()
        assert returned["event"] == "return_completed"
        assert returned["slot"]["status"] == "available"

        wallet = client.get(f"/api/users/{user['id']}/wallet").json()
...
def test_defect_report_penalizes_previous_user(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        client.post("/api/hardware/qr/scan", json={"token": qr_a["token"]})
        client.post("/api/hardware/slots/1/sensor", json={"present": False})
        return_qr_a = client.post("/api/qr/return", json={}, headers=headers_a).json()
        client.post("/api/hardware/qr/scan", json={"token": return_qr_a["token"]})
        client.post("/api/hardware/slots/1/sensor", json={"present": True})

        qr_b = client.post("/api/qr/rent", json={"slot_id": 1}, headers=headers_b).json()
        scan_b = client.post("/api/hardware/qr/scan", json={"token": qr_b["token"]}).json()
        client.post("/api/hardware/slots/1/sensor", json={"present": False})
        ...
        wallet_a = client.get(f"/api/users/{user_a['id']}/wallet").json()
        wallet_b = client.get(f"/api/users/{user_b['id']}/wallet").json()
...
def test_slot_report_and_damage_return_flow(tmp_path):
    ...
    with TestClient(app) as client:
        ...
        scan = client.post("/api/hardware/qr/scan", json={"token": qr["token"]}).json()
        client.post(f"/api/hardware/slots/{slots[0]['id']}/sensor", json={"present": False})
        ...
        client.post("/api/hardware/qr/scan", json={"token": return_qr["token"]})
        returned = client.post(f"/api/hardware/slots/{slots[0]['id']}/sensor", json={"present": True}).json()
